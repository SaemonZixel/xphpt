...
function system_with_timeout($commandline, $env = null, $stdin = null)
{
	...
	$bin_env = array('DISPLAY' => ':0');
	foreach((array)$env as $key => $value) {
		$bin_env[$key] = $value;
	}
	
	$proc = proc_open("cd ".exec('pwd')."; $commandline", array(
		0 => array('pipe', 'r'),
		1 => array('pipe', 'w'),
		2 => array('pipe', 'w')
...
// 					}
			...
			if ((@count($section_text['EXPECT']) + @count($section_text['EXPECTF']) + @count($section_text['EXPECTREGEX'])) + @count($section_text['EXPECTREGEX_VARIANT1'])  + @count($section_text['EXPECT_VARIANT1']) != 1 ) {
...
";
	...
	if(preg_match("~^--(EXPECT[^_]*)(_VARIANT[0-9]+)--\n(.*)~s", $output, $matches)) {
		$section_text[$matches[1]] = $section_text[$matches[1].$matches[2]];
		$output = $matches[3];
// var_dump($matches, $section_text[$matches[1]], $section_text[$matches[1].$matches[2]]);
	}
	
	show_file_block('out', $output);

	foreach (array( 'EXPECT', 'EXPECTF', 'EXPECTREGEX' ) as $prefix)
		if(isset($section_text[$prefix . '_EXTERNAL'])) {
			$section_text[$prefix] = file_get_contents($section_text[$prefix . '_EXTERNAL'], FILE_BINARY);
			
			if(strpos($section_text[$prefix . '_EXTERNAL'], '.remove') or strpos($section_text[$prefix . '_EXTERNAL'], '.del')) 
				unlink($section_text[$prefix . '_EXTERNAL']);
		}
